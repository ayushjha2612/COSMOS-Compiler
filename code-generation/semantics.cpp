...
void handleFunctionCall(bool hasZeroArgs = false)
{
    ...
    else
    {
        if (S->symType == FUNCTION)
        {
            ...
            {
                printError("Argument numbers do not match expected " + to_string(S->arglist.size()) + " arguments but got " + to_string(argList.size()), false);
            }
            if (!hasZeroArgs)
            {
                ...
                for (auto &s : argList)
                {
                    actTypes.push_back(s.second);
                    if (exprfuncName == "input")
                    {
                        inputs.push(s.first);
                        if (s.first == "CONST")
                        {
                            printError("Taking inputs in a constant", false);
                        }
                    }
                    if (exprfuncName == "output")
                    {
                        outputs.push(s.first);
                    }
                }
                for (auto &s : S->arglist)
                {
                    expTypes.push_back(typeConst(s.second));
                }
                int index = min(expTypes.size(), actTypes.size());
                for (int i = 0; i < index; i++)
                {
                    if (expTypes[i] != actTypes[i] && expTypes[i] != "ANY")
                    {
...
void handleArithExpr(string opr = "")
{
    ...
    else
    {
        ...
        if ((exprTerms.first == exprTerms.second && exprTerms.second == "SN") || (exprTerms.first.find("STRUCT") != exprTerms.first.npos) || (exprTerms.second.find("STRUCT") != exprTerms.second.npos))
        {
...
{
    exprTerms.second = exprType;
    if (exprTerms.first == exprTerms.second || (exprTerms.first.find("STRUCT") != exprTerms.first.npos) || (exprTerms.second.find("STRUCT") != exprTerms.second.npos))
    {
        exprTerms.second.clear();
        if (logicalLhs == "")
            logicalLhs = "BOOL";
...
void handleAssignExpr()
{
    ...
    {
        exprType = typeConst(S->dataType);
        if (exprType != exprTerms.first && (exprTerms.first.find("STRUCT") == exprTerms.first.npos) && (exprType.find("STRUCT") == exprType.npos))
        {
...
//     {
//         printError("Types do not match on both sides of assignment RHS expected " + idenType + " but got " + constType);
//     }
// }
...
void checkUnusedVars()
{
    ...
    {
        if (!a.second.isUsed && a.second.symType != SNCONST)
        {
    ...
    for (auto &b : symboltable.functables)
    {
        ...
        {
            if (!c.second.isUsed && c.second.symType != SNCONST)
            {
...
void handleIO()
{
    symboltable.insert("input", "VOID", FUNCTION, "GLOBAL");
    symboltable.insert("output", "VOID", FUNCTION, "GLOBAL");

    symbolEntry *S = symboltable.search("input");
    list<pair<string, string>> args;
    args.push_back({"arg", "ANY"});
    S->arglist = args;
    S = symboltable.search("output");
    S->arglist = args;
}
// int main()
// {
